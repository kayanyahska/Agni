...
@app.route('/returnSign/<int:aadhaar_no>',methods={'GET'})
...
def returnSign(aadhaar_no):
	...
	global auth
	auth = str({'Aadhaar Number':aadhaar_no,'Public Key': str(pem),'Signature': str(sig)})
	return auth
...
@app.route('/esign/<int:aadhaar_no>',methods={'GET'})
def esign(aadhaar_no,json1):
...
#@app.route('/sign_message/<string:s>',methods={'GET'})
